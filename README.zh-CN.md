...
- `data/config.yml`：保存限流配置与 DeepSeek 聊天行为配置，支持注释。
- `data/nanabot.db`：保存群白名单和用户白名单。
- 首次升级到该版本后，旧 `data/config.json` 会自动迁移为 `data/config.yml`，其中旧 JSON 里的白名单数据也会自动迁移到
  SQLite。
